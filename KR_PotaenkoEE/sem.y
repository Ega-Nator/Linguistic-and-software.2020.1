...
%token KEY_INTEGER KEY_LONG_INTEGER KEY_BEGIN KEY_END CONSTANT IDENTIFIER MULTIPLY DIVIDE MINUS PLUS SEMICOLON COMMA ASSIGN OPEN_BRACKET CLOSE_BRACKET DELIMITER
...
DESCRIPTION_OF_CALCULATION:
	...
	{printf($1); printf("%s","\n"); printf("%s",$2); printf("%s",$3); printf("."); printf("%s","\n");}
;

VARIABLE_DECLARATION:
	KEY_INTEGER VARIABLE_LIST
	{printf("%s%s", "Var"," "); printf("%s%s",$2); printf(":"); printf(" "); printf("%s%s",$1," "); printf("%s","\n");}
        | KEY_LONG_INTEGER VARIABLE_LIST
        {printf("%s%s", "Var"," "); printf("%s%s",$2); printf(":"); printf(" "); printf("%s%s",$1," "); printf("%s","\n");}
;
